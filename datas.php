...
function calculaDias($dataInicial, $dataFinal) {
	/*
	- Setembro, abril, junho e novembro tem 30 dias, todos os outros meses tem 31 exceto fevereiro que tem 28, exceto nos anos
	bissextos nos quais ele tem 29.
	- Os anos bissexto tem 366 dias e os demais 365.
	- Todo ano divisivel por 4 e um ano bissexto.
	- A regra acima não e valida para anos divisiveis por 100. Estes anos devem ser divisiveis por 400 para serem anos
	bissextos. Assim, o ano 1700, 1800, 1900 e 2100 nao sao bissextos, mas 2000 e bissexto.
	...
	*/
	...
	$bissextoIni   = $anoInicial % 400;
	$bissextoFinal = $anoFinal % 400;

	$countYears = $anoFinal - $anoInicial;
	$countMounths = $mesFinal - $mesInicial;
	$countDays = $diaFinal - $diaInicial;


	$to_date = strtotime($inicial);
	$from_date = strtotime($final);
	// $to_date = 915156000;
	// $from_date = 978314400;
	$dayDiff = $to_date - $from_date;
	$days = abs($dayDiff/(60*60*24));
	$days = round($days);
	return $days;
	// return $to_date . " = " .$from_date;
	// return ($countYears. " a " .$countMounths." m. ").($countDays." d.").$timeStamp.PHP_EOL;
